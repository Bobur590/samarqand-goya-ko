    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 py-10 md:py-14">
        <div className="mx-auto max-w-3xl px-4">
          <h1 className="text-2xl font-bold text-foreground md:text-3xl">Platforma qanday ishlaydi?</h1>
          ...
          <div className="mt-10 space-y-8">
            <section>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-lg font-semibold text-foreground">AI Scoring tizimi</h2>
              </div>
              ...
              <div className="space-y-2">
                {SCORE_CRITERIA.map((c) => (
                  <div key={c.key} className="flex items-center justify-between rounded-lg border bg-card p-3">
                    ...
                    <span className="text-sm font-bold text-primary">{c.max} ball</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-lg font-semibold text-foreground">Statuslar</h2>
              </div>
              <div className="grid gap-2 sm:grid-cols-2">
                {[
                  { range: "85–100", label: "TOP", color: "bg-gold/15 border-gold/30" },
                  { range: "70–84", label: "Yaxshi", color: "bg-success/15 border-success/30" },
                  { range: "50–69", label: "Oddiy", color: "bg-warning/15 border-warning/30" },
                  { range: "0–49", label: "Reject", color: "bg-reject/15 border-reject/30" },
                ].map((s) => (
                  <div key={s.label} className={`rounded-lg border p-3 ${s.color}`}>
                    <span className="font-bold text-foreground">{s.label}</span>
                    <span className="ml-2 text-sm text-muted-foreground">{s.range} ball</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-lg font-semibold text-foreground">Adolat kafolati</h2>
              </div>
